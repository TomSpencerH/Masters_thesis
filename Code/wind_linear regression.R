...
slope_data <- slope_func(df = buff_wind)

r2 <- function(df){
  
  u_r <- df %>% 
    group_by(lon, lat) %>% 
    group_modify(~ broom::glance(lm(u ~ season, data = .x))) %>% 
    select(lon, lat, r.squared) %>% 
    rename(u_r2 = r.squared)
  
  v_r <- df %>% 
    group_by(lon, lat) %>% 
    group_modify(~ broom::glance(lm(v ~ season, data = .x))) %>% 
    select(r.squared) %>% 
    rename(v_r2 = r.squared)
  
  spd_r <- df %>% 
    group_by(lon, lat) %>% 
    group_modify(~ broom::glance(lm(wind_spd ~ season, data = .x))) %>% 
    select(r.squared) %>% 
    rename(spd_r2 = r.squared)
  
  dir_r <- df %>% 
    group_by(lon, lat) %>% 
    group_modify(~ broom::glance(lm(wind_dir ~ season, data = .x))) %>% 
    select(r.squared) %>% 
    rename(dir_r2 = r.squared)
  
  aws_r <- df %>% 
    group_by(lon, lat) %>% 
    group_modify(~ broom::glance(lm(AWS ~ season, data = .x))) %>% 
    select(r.squared) %>% 
    rename(aws_r2 = r.squared)
  
  curl_r <- df %>% 
    group_by(lon, lat) %>% 
    group_modify(~ broom::glance(lm(curl ~ season, data = .x))) %>% 
    select(r.squared) %>% 
    rename(curl_r2 = r.squared)
  
  slope <- u_r %>% 
    left_join(v_r, by = c("lon", "lat")) %>% 
    left_join(spd_r, by = c("lon", "lat")) %>% 
    left_join(dir_r, by = c("lon", "lat")) %>% 
    left_join(aws_r, by = c("lon", "lat")) %>% 
    left_join(curl_r, by = c("lon", "lat"))
  
  return(slope)
}


slope <- r2(df = buff_wind)

slope_new <- slope_data %>% 
  left_join(slope, by = c("lon", "lat"))

  
ggplot(aws, aes(x = lon, y = lat)) +
 ...
 ggplot(slope_new, aes(x = lon, y = lat)) +
  geom_tile(aes(fill = v_slope)) +
   geom_density_2d(data = slope_new %>% 
                    filter(v_p.value <= 0.05), aes(x = lon, y = lat), bins = 5, col = "black", alpha = 0.5) +
  borders("world", regions = c("South Africa", "Namibia")) +
  coord_fixed(xlim = c(11, 21), ylim = c(-35, -17)) +
  scale_fill_viridis(option = "turbo")

range(slope_data$v_slope)

library(viridis)
 p5 <- ggplot(slope_data, aes(x = lon, y = lat)) +
  geom_tile(aes(fill = v_slope*10)) +
  borders("world", regions = c("South Africa", "Namibia")) +
  coord_fixed(xlim = c(11, 21), ylim = c(-35, -17)) +
    geom_contour(data = slope_data %>% 
                   filter(v_p.value < 0.05), 
                 aes(x = lon, y = lat, z = v_slope), bins = 2, col = "black") +
    scale_fill_continuous_diverging(palette = "Green-Brown", l1 = 20, l2 = 90, p1 = 0.7, p2 = 1,
                                    rev = FALSE,
                                    limits = c(-0.3, 0.3),
                                    # guide = "colourbar",
...
annotate_figure(plot, top = text_grob("Wind Variables - JJA (1981 - 2023)", 
